...
import { TENIENTES, getRango } from '../../../../data/roles' 
import {
...
export default function Turnos() {
  ...
  const cambiarEncargado = (idPlanilla, nuevoEncargado) => {
    setPlanillas((prev) => prev.map((pl) => pl.id === idPlanilla ? { ...pl, encargado: nuevoEncargado } : pl))
  }
  ...
  const guardarBloque = (idPlanilla, idBloque, cambios) => {
    setPlanillas((prev) =>
      prev.map((pl) => {
        if (pl.id !== idPlanilla) return pl
        return { ...pl, bloques: pl.bloques.map((b) => b.id === idBloque ? { ...b, ...cambios } : b) }
      })
    )
    setEditando(null)
  }
  ...
  const eliminarBloque = (idPlanilla, idBloque) => {
    setPlanillas((prev) =>
      prev.map((pl) => pl.id === idPlanilla ? { ...pl, bloques: pl.bloques.filter((b) => b.id !== idBloque) } : pl)
    )
  }

  const eliminarTarea = (idPlanilla, indiceTarea) => {
    setPlanillas((prev) =>
      prev.map((pl) => pl.id === idPlanilla ? { ...pl, tareas: pl.tareas.filter((_, i) => i !== indiceTarea) } : pl)
    )
  }
  ...
  return (
    <>
      <div className="vista-head">
        <h1>Turnos</h1>
        <p>Planillas de asistencia semanal. La asignación de turnos rota cada semana entre los tenientes.</p>
      </div>

      {puedeCrearGuardia && (
        <div className="turnos-acciones">
          <button className="btn btn-primario" onClick={() => setCreandoGuardia(true)}>
      ...
      {puedeMarcar ? (
        <div className="nota-info">
          <IconoCheck width={18} />
          Anótate en los bloques que puedas cubrir. Al completarse los cupos, el bloque se cierra y se muestran los seleccionados.
        </div>
      ) : (
        <div className="nota-info">
          <IconoOjo width={18} />
          Tu rango <strong>&nbsp;visualiza&nbsp;</strong> las planillas; no requiere marcar asistencia.
        </div>
      )}
        ...
        return (
          <section className="planilla" key={pl.id}>
            <header className="planilla__head">
              <div>
                <h2>{pl.titulo}</h2>
                <span className="planilla__sub"><IconoCalendario width={14} /> {pl.subtitulo}</span>
              </div>
            ...
            <div className="planilla__acciones">
              <button className="btn-mini" onClick={() => descargarPDF(pl)} title="Descargar turno como PDF">
                <IconoDescargaPDF width={14} /> Descargar PDF
              </button>
              {puedeEditar && (
                <button className="btn-mini btn-mini--peligro" onClick={() => eliminarPlanilla(pl.id)} title="Eliminar turno">
            ...
            <div className="planilla__encargado">
              <span className="planilla__encargado-rotulo"><IconoPersona width={15} /> Encargado de turno</span>
              {puedeEditar ? (
                <select value={pl.encargado} onChange={(e) => cambiarEncargado(pl.id, e.target.value)} className="planilla__encargado-select">
                  {TENIENTES.map((t) => (
                    <option key={t.id} value={t.id}>{t.numero} · {t.persona}</option>
                  ))}
                </select>
              ) : (
                <strong className="planilla__encargado-nombre">{encargado.numero} · {encargado.persona}</strong>
              )}
            </div>

            {pl.tareas && pl.tareas.length > 0 && (
              <div className="planilla__tareas">
                <span className="planilla__tareas-rotulo"><IconoCheck width={14} /> Tareas de la guardia</span>
                <ul>
                  {pl.tareas.map((t, i) => (
                    <li key={i}>
                      <span>{t}</span>
                      {puedeEditar && (
                        <button className="planilla__tarea-quitar" onClick={() => eliminarTarea(pl.id, i)} aria-label="Eliminar tarea">
            ...
            <ul className="bloques">
              {pl.bloques.map((b) => {
                ...
                const enEdicion = editando && editando.planillaId === pl.id && editando.bloqueId === b.id

                if (enEdicion) {
                  return (
                    <li key={b.id} className="bloque bloque--editando">
                      <FormEdicionBloque bloque={b} onGuardar={(cambios) => guardarBloque(pl.id, b.id, cambios)} onCancelar={() => setEditando(null)} />
                    </li>
                  )
                }
                ...
                return (
                  <li
                    key={b.id}
                    className={`bloque ${b.inscritoYo ? 'bloque--mio' : ''} ${cerrado ? 'bloque--cerrado' : ''} ${puedeMarcar && !cerrado ? 'bloque--marcable' : ''}`}
                    onClick={() => !cerrado && alternarBloque(pl.id, b.id)}
                    role={puedeMarcar && !cerrado ? 'button' : undefined}
                    tabIndex={puedeMarcar && !cerrado ? 0 : undefined}
                    onKeyDown={(e) => puedeMarcar && !cerrado && e.key === 'Enter' && alternarBloque(pl.id, b.id)}
                  >
                    <span className="bloque__check">
                      {cerrado ? <IconoCandado width={14} /> : b.inscritoYo ? <IconoCheck width={15} /> : <i />}
                    </span>
                    <div className="bloque__info">
                      <div className="bloque__titulo">
                        ...
                        {cerrado && <span className="bloque__cerrado-tag">Completo</span>}
                      </div>
                      <div className="bloque__barra">
                        <div className={`bloque__relleno ${cerrado ? 'bloque__relleno--lleno' : ''}`} style={{ width: `${porcentaje}%` }} />
                      </div>
                      {cerrado && (
                        <ul className="bloque__seleccionados">
                          {b.anotados.slice(0, b.cupos).map((n, idx) => (
                            <li key={idx}><IconoCheck width={12} /> {n}</li>
                          ))}
                        </ul>
                      )}
                    </div>
                    <span className="bloque__conteo">{anotados}/{b.cupos}</span>
                    {puedeEditar && (
                      <span className="bloque__acciones">
                        <button className="bloque__editar" onClick={(e) => { e.stopPropagation(); setEditando({ planillaId: pl.id, bloqueId: b.id }) }} aria-label="Editar bloque" title="Editar cupos y horario">
                          <IconoLapiz width={15} />
                        </button>
                        <button className="bloque__eliminar" onClick={(e) => { e.stopPropagation(); eliminarBloque(pl.id, b.id) }} aria-label="Eliminar bloque" title="Eliminar este bloque">
      ...
      {creandoGuardia && (
        <FormCrearGuardia rangoActual={rangoActual} onCrear={crearGuardia} onCerrar={() => setCreandoGuardia(false)} />
      )}
    </>
  )
}
  ...
  return (
    <div className="edicion-bloque">
      <div className="edicion-bloque__campos">
        <label>Día<input value={bloque.dia} disabled /></label>
        <label>Horario<input value={horario} onChange={(e) => setHorario(e.target.value)} placeholder="09:30 a 13:30" /></label>
        <label>Cupos<input type="number" min={bloque.anotados.length} value={cupos} onChange={(e) => setCupos(Number(e.target.value))} /></label>
      </div>
      <div className="edicion-bloque__acciones">
        <button className="btn-mini" onClick={onCancelar}>Cancelar</button>
        <button className="btn-mini btn-mini--primario" onClick={() => onGuardar({ cupos, horario })}>Guardar</button>
      </div>
    </div>
  )
}